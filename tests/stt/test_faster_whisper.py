...
@pytest.fixture
def settings():
    return load_settings(api_key="k", env={}, stt_provider="faster-whisper")
...
def test_provider_attrs_and_default_model(settings):
    stt = FasterWhisperSTT(settings, decode=lambda p: ("x", "en", 1.0))
...
def test_model_override_is_namespaced():
    s = load_settings(env={}, stt_provider="faster-whisper", stt_model="large-v3")
    assert FasterWhisperSTT(s, decode=lambda p: ("", "en", 1.0)).model == "faster-whisper:large-v3"


def test_transcribe_maps_language_and_strips(settings, wav):
    stt = FasterWhisperSTT(settings, decode=lambda p: ("  नमस्ते  ", "hi", 0.87))
...
def test_calls_are_serialized(settings, wav):
    ...
    stt = FasterWhisperSTT(settings, decode=decode)
...
def test_real_engine_smoke(settings, tmp_path):
    ...
    r = FasterWhisperSTT(settings).transcribe(wav)
